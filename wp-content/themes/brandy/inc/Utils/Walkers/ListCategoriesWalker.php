...
class ListCategoriesWalker extends \Walker_Category {
	public function start_el( &$output, $data_object, $depth = 0, $args = array(), $current_object_id = 0 ) {
		...
		if ( ! empty( $args['show_count'] ) ) {
			$cat_name .= ' <span class="cat-item-count">' . number_format_i18n( $category->count ) . '</span>';
		}
		...
		if ( ! empty( $args['feed_image'] ) || ! empty( $args['feed'] ) ) {
			...
			if ( empty( $args['feed'] ) ) {
				...
				$alt = ' alt="' . sprintf( __( 'Feed for all posts filed under %s', 'brandy' ), $cat_name ) . '"';
			} else {
				$alt   = ' alt="' . $args['feed'] . '"';
				$name  = $args['feed'];
